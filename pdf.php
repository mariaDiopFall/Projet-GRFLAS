...
function PutLink($URL, $txt)
{
	...
	$this->Image('img/'.$_SESSION['prenom'].'.png',130,255,70);
		...
		$this->SetX(40);
		// Texte : 90 >largeur ligne, 8 >hauteur ligne. Premier 0 >pas de bordure, 1 >retour à la ligneensuite, C >centrer texte, 1> couleur de fond ok	
		$this->Cell(50,8,"Reference RCR:".' ' ,0,1,'C',1);
		//$this->Cell(50,8,"Reference RCR:".' '.$data_rapport['num_rapport'] ,0,1,'C',1);
		// Saut de ligne 10 mm
		$this->Ln(10);
...
while ($data_rapport = mysqli_fetch_array($result)) {
	...
	$pdf->SetY($position_detail);
	$pdf->SetX(10);
	//$pdf->MultiCell(100,10,utf8_decode('Tiers1:'.$data_rapport['situational_awar_sectionan1'])."\n".utf8_decode('Tiers2:'.$data_rapport['situational_awar_sectionan2'])."\n".utf8_decode('Tiers3:'.$data_rapport['situational_awar_sectionan3']),1,'C');
	$pdf->MultiCell(100,10,utf8_decode($data_rapport['situational_awar_sectionan1'])."\n".utf8_decode($data_rapport['situational_awar_sectionan2'])."\n".utf8_decode($data_rapport['situational_awar_sectionan3'])."\n".utf8_decode($data_rapport['manoeuvrean']),1,'C');
...
function entete_table2($position_entete3) {
	...
	// position de colonne 1 (10mm à gauche)	
	$pdf->SetX(10);
	$pdf->Cell(100,10,'Partie Consciente de la Situation',1,0,'C',1);	// 60 >largeur colonne, 8 >hauteur colonne
	// position de la colonne 2 (70 = 10+60)
	$pdf->SetX(110); 
...
while ($data_rapport = mysqli_fetch_array($result)) {
	...
	$pdf->SetY($position_detail);
	$pdf->SetX(10);
	//$pdf->MultiCell(100,10,utf8_decode('Tiers1:'.$data_rapport['situational_awar_section1'])."\n".utf8_decode('Tiers2:'.$data_rapport['situational_awar_section2'])."\n".utf8_decode('Tiers3:'.$data_rapport['situational_awar_section3']),1,'C');
	$pdf->MultiCell(100,10,utf8_decode($data_rapport['situational_awar_section1'])."\n".utf8_decode($data_rapport['situational_awar_section2'])."\n".utf8_decode($data_rapport['situational_awar_section3'])."\n".utf8_decode($data_rapport['manoeuvre']),1,'C');
...
mysqli_free_result($result);
  ...
  $pdf->Image("envoii.png",10,270,20,0,"","envoiepdf.php");
  $pdf->Image("retour.png",180,2,20,0,"","etatpiste.php");
...
$pdf->Output('RapportPiste'.$date.'.pdf', 'I');
...
/*$mail->setFrom('mariama.diop@dakaraeroport.com', 'Mailer'); // Personnaliser l'envoyeur
$mail->addAddress('mariama.diop@dakaraeroport.com', 'Karim User'); // Ajouter le destinataire
...
<body>
<form><input type="button" onclick="envoie()" value="Envoi Rapport"></form>

<script type="text/javascript"> 
 function envoie() {

	var answer = confirm("Voulez vous envoyer le rapport?")
	
	if (answer){
	
		window.location = "envoiepdf.php";
	
	}
	else{
		window.location= "etatpiste.php"
	}
